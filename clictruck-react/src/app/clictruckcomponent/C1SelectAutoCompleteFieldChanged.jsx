...
import C1Version from "../c1component/C1Version";
 ...
 */
const C1SelectAutoCompleteFieldChanged = ({
       label,
       name,
       onChange,
       value,
    ...
    useEffect(() => {
        ...
        if (dataList.length === 0) {
            ...
            if(isShowOtherOption) {
                setOptionsSelection(otherOptions)
                if (value) {
                    let val = '';
                    Object.keys(otherOptions).forEach(element => {
                        if (otherOptions[element][valueKey] === value) {
                            val = isShowCode ? `${otherOptions[element].value} - ${otherOptions[element].desc}` : otherOptions[element].desc;
                        }
                    });
                    setInputValue(val);
                }
    ...
    useEffect(() => {
        if(!isServer && optionsMenuItemArr && optionsMenuItemArr.length > 0) {
            setOptionsSelection(optionsMenuItemArr);

            if (value) {
                let val = '';
                Object.keys(optionsMenuItemArr).forEach(element => {
                    if (optionsMenuItemArr[element].value === value) {
                        val = isShowCode ? `${optionsMenuItemArr[element].value} - ${optionsMenuItemArr[element].desc}` : optionsMenuItemArr[element].desc;
                    }
                });
                setInputValue(val);
            }
        }
    },[optionsMenuItemArr, value])
...
C1SelectAutoCompleteFieldChanged.propTypes = {
...
export default C1SelectAutoCompleteFieldChanged;