...
const AuditFormDetail = () => {
    ...
    useEffect(() => {
        setLoading(false);
        if (viewType !== 'new') {
            sendRequest("/api/co/common/entity/auditLog/" + id, "getAudit", "get", {});
        }
    ...
    let bcLabel = t("administration:audit.details.view");

    return (
        loading ? <MatxLoading /> : <React.Fragment>
            <C1FormDetailsPanel
                breadcrumbs={[
                    { name: t("administration:audit.list.routeSegment"), path: "/administrations/audit/list" },
                    { name: bcLabel },
                ]}
...
export default withErrorHandler(AuditFormDetail);