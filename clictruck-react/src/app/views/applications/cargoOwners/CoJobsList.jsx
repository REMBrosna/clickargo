...
import { Status, JobStates, CK_MST_SHIPMENT_TYPE } from "app/c1utils/const";
...
const CoJobsList = () => {
    ...
    const columns = [
        {
            name: "jobId",
            label: t("listing:common.jobId")
        },
        {
            name: "tckDoi.doiBlNo",
            label: t("listing:coJob.blNo")
        },
        {
        ...
        {
            name: "tcoreAccnByJobAuthorizedPartyAccn.accnName",
            label: t("listing:coJob.party")
        },
        {
            name: "tckDoi.tckDo.tcoreAccn.accnId",
            label: t("listing:common.shippingLine")
        },
        {
            name: "tckJob.tckRecordDate.rcdDtSubmit",
            label: t("listing:common.dtSubmitted"),
            options: {
                filter: true,
                filterType: 'custom',
                display: true,
            ...
            options: {
                ...
                customBodyRender: (value, tableMeta, updateValue) => {
                    ...
                    switch (value) {
                        ...
                        case JobStates.SUB.code://JIRA CLICDO-232 CO Job submission does not call DODetails immediately, decouple API invocation
                            if(tableMeta.rowData[8] === "UNRDY"){
                                return <small className="px-1 py-2px border-radius-4" style={{ backgroundColor: cyan[200], color: cyan[800] }}>
                                    *Submitting
                                </small>;
                            }else{
        ...
        },
        {
            ...
            options: {
                ...
                customBodyRender: (value, tableMeta, updateValue) => {
                    ...
                    // return <Grid container alignItems="flex-start" justifyContent="flex-start"><C1DataTableActions
                    return<C1DataTableActions
                    // editPath={status === Status.NEW.code ? `/applications/services/co/job/edit/${jobId}` : null}
                    // viewPath={`/applications/services/co/job/view/${jobId}`}
                    // removeEventHandler={status === Status.NEW.code ? (e) => handleDeleteConfirm(e, jobId) : null}
                    // cancelEventHandler={status === Status.SUB.code ? (e) => handleCancelConfirm(e, jobId) : null}
                    >
                                ...
                                <Grid container item alignItems="center">
                                    <Grid item xs={4}>
                                        <span style={{ minWidth: '48px' }}>
                                            {status === Status.NEW.code && <Link to={`/applications/services/co/job/edit/${jobId}`}>
        ...
        //8
        {
            name: "tckDoi.tckDo.tckMstDoState.dostId",
            label: " ",
            options: {
                display: 'excluded',
                filter: false,
    ...
    const redirectAddHandler = () => {
        ...
        history.push("/applications/services/co/job/new/-");
    };
    ...
    const handleDeleteHandler = (e) => {
        ...
        if (openDeleteConfirm && openDeleteConfirm.action === "DELETE") {
            setOpen(false);
            sendRequest("/api/v1/clickargo/clicdo/job/doiCo/" + confirm.jobId, "delete", "delete", {});
        } else if (openDeleteConfirm && openDeleteConfirm.action === "CANCEL") {
            setOpen(false);
            sendRequest("/api/v1/clickargo/clicdo/job/doiCo/" + confirm.jobId, "cancelJob", "get", null);
        }
    }
    ...
    return (<React.Fragment>
        ...
        <C1ListPanel
            routeSegments={[
                { name: t("listing:coJob.clicDoJobListTitle") }
