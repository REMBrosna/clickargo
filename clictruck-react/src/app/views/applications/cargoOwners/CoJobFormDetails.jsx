...
import CoJobAuthLetters from "./tabs/CoJobAuthLetters";
import CoJobDetails from "./tabs/CoJobDetails";
...
const CoJobFormDetails = () => {
    ...
    const tabList = [
        { text: t("cargoowners:tabs.jobDetails"), icon: <WorkOutlineOutlinedIcon /> },
        { text: t("cargoowners:tabs.attachments"), icon: <AttachFileOutlinedIcon /> },
        { text: t("cargoowners:tabs.audit"), icon: <AccessTimeOutlinedIcon /> }
    ];
    ...
    useEffect(() => {
        ...
        if (!isLoading && !error && res && !validation) {
            ...
            switch (urlId) {
                case "newJob": {
                    setInputData({ ...res?.data, "documentType": "BL" });//setting default for documentType
                    const reqBody = {
                        entityType: "JOB_DOI_CO",
                ...
                case "createJob": {
                    ...
                    setSnackBarOptions({
                        ...
                        redirectPath: "/applications/services/co/job/edit/" + res?.data?.jobId,
                    });
                    break;
                }
                case "getJob": {
                ...
                case "isSelectedPartyAuthorized":
                    setInputData({ ...inputData, ...deepUpdateState(inputData, ["authorized"], res.data) });
                    // console.log(inputData?.tckJob?.tckMstJobState?.jbstId)
                    if (inputData?.tckJob?.tckMstJobState?.jbstId === JobStates.NEW.code || inputData?.tckJob?.tckMstJobState?.jbstId === JobStates.SUB.code) {
                        const reqBody = {
                            entityType: "JOB_DOI_CO",
                            entityState: inputData?.tckJob?.tckMstJobState?.jbstId,
                            // entityState: "SUB", //temporary test
                ...
                // case "uploadBL": {
                //     console.log("uploadBL response", res);
                //     setInputData({ ...inputData, attchId: res?.data?.jobAttach?.attId })
                //     break;
                // }
                case "download": {
    ...
    const handleSaveOnClick = () => {
        ...
        switch (viewType) {
            case "new":
                sendRequest("/api/v1/clickargo/clicdo/job/doiCo", "createJob", "post", { ...inputData });
                break;
            case "edit":
                sendRequest("/api/v1/clickargo/clicdo/job/doiCo/" + jobId, "updateJob", "put", { ...inputData });
                break;
            default:
                break;
        }
    ...
    return loading ? <MatxLoading /> : (<React.Fragment>
        <C1FormDetailsPanel
            breadcrumbs={[
                { name: t("cargoowners:form.clicDoJobList"), path: "/applications/services/co/list" },
                    ...
                    <Grid item xs={12}>
                        <Paper>
                            ...
                            {tabIndex === 0 && <C1TabInfoContainer guideId='clicdo.doi.co.jobs.tabs.details' title="empty" guideAlign="right" open={false}>
                                <CoJobDetails viewType={viewType}
                            ...
                            {tabIndex === 1 &&
                                <C1TabInfoContainer guideId='clicdo.doi.co.jobs.tabs.authorisation' title="empty" guideAlign="right" open={false}>
                                    <CoJobAuthLetters viewType={viewType}
                                        inputData={inputData} />
                                </C1TabInfoContainer>}
...
export default withErrorHandler(CoJobFormDetails);