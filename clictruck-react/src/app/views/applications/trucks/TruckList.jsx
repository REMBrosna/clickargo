...
const TruckList = () => {
    ...
    const [alreadyCancelledErrorOpen, setAlreadyCancelledErrorOpen] = useState({ msg: null, open: false });
    ...
    const columns = [
        ...
        // 6
        {
            name: "pickUp.tlocLocName",
            label: t("listing:trucklist.pickup"),
        },
        // 7
        {
            name: "lastDrop.tlocLocName",
        ...
        if (!isLoading && !error && res) {
            switch (urlId) {
                case "delete":
                    ...
                    break;
                case "cancelJob":
                    sendRequest("/api/v1/clickargo/clictruck/job/truck/" + confirm.jobId, "cancelled", "put", { ...res.data, "action": "CANCEL" });
                    break;
                case "cancelled":
                    ...
                    if (res?.data?.cancelled === true) {
                        setAlreadyCancelledErrorOpen({
                            ...alreadyCancelledErrorOpen,
                            msg: "Job has already been cancelled by another user!", open: true
                        });
                        return;
                    } else {
    ...
    return (<React.Fragment>
        ...
        <C1DialogPrompt confirmationObj={{
            openConfirmPopUp: alreadyCancelledErrorOpen?.open,
            onConfirmationDialogClose: () => { },
            text: alreadyCancelledErrorOpen?.msg,
            title: "Job Already Cancelled",
            onYesClick: () => setAlreadyCancelledErrorOpen({ ...alreadyCancelledErrorOpen, open: false }),
