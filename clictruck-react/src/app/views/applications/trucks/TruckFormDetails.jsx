...
const TruckFormDetails = () => {
        ...
        if (!isLoading && !error && res && !validation) {
            ...
            switch (urlId) {
                ...
                case "getJob": {
                    ...
                    const reqBody = {
                        entityType: "JOB_TRUCK",
                        entityState: data?.tckJob?.tckMstJobState?.jbstId,
    ...
    const handleExitOnClick = () => {
        history.push("/applications/services/truck/list");
    }
    ...
    const handleSaveOnClick = () => {
        ...
        switch (viewType) {
            case "new":
                ...
                // sendRequest(`${truckUrlApi}`, "createJob", "POST", { ...inputData, "tckJob": { ...inputData['tckJob'], "tckMstShipmentType": { "shtId": 'EXPORT' } } });
                break;
            case "edit":
                    ...
                    <Grid item xs={12}>
                        <Paper>
                            ...
                            {tabIndex === 0 && <C1TabInfoContainer guideId='clicdo.doi.co.jobs.tabs.details' title="empty" guideAlign="right" open={false}>
                                <TruckJobDetails viewType={viewType}
                            ...
                            {tabIndex === 1 &&
                                <C1TabInfoContainer guideId='clicdo.doi.co.jobs.tabs.authorisation' title="empty" guideAlign="right" open={false}>
                                    <TruckJobAuthLetters viewType={viewType}
                                        inputData={inputData} />
                                </C1TabInfoContainer>}
...
export default withErrorHandler(TruckFormDetails);