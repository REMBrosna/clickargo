...
const JobPayments = ({ }) => {
                            ...
                            onChange={({ target: { checked } }) => {
                                try {
                                    if (checked === true) {
                                        ...
                                        setSelectedPaymentStatus(statusData)
                                    } else {
                                        setSelectedRowIds([])
                            ...
                            <Grid item xs={4} >
                                <C1LabeledIconButton tooltip="Payment Details" label={t("buttons:view")}
                                    action={() => history.push({
                                        ...
                                        state: { from: "/applications/services/gli/dashboard" }
                                    })}>
    ...
    const handleGliValidation = React.useMemo(() => {
        let str = ""
        const verifier = user?.authorities?.some((item) => item.authority === Roles.FINANCE_VERIFIER.code)
        const approver = user?.authorities?.some((item) => item.authority === Roles.FINANCE_APPROVER.code)
        const newPayment = selectedPaymentStatus.some((item) => item?.status === JobStates.NEW.code)
        const verPayment = selectedPaymentStatus.some((item) => item?.status === JobStates.VER_BILL.code)

        if (verifier && approver) {
            if (newPayment && verPayment) {
                str = Roles.FINANCE_APPROVER.code
            } else {
                if (newPayment) {
                    str = Roles.FINANCE_VERIFIER.code
                } else {
                    str = Roles.FINANCE_APPROVER.code
                }
            }
    ...
    const handleOpenPopover = (e) => {
        ...
        if (selectedRowIds.length === 0) {
            ...
            sendRequest(`api/v1/clickargo/clictruck/payments/action?accnType=${AccountTypes.ACC_TYPE_SP.code}&role=${handleGliValidation}&paymentId=${query}`, "getMultiOptions", "get")
        }
    }
            ...
            switch (urlId) {
                ...
                case "getMultiOptions": {
                    ...
                    if (dataOpt?.actions?.length > 0) {
                        ...
                        setAnchorEl(null)
                        // setSelectedRowIds([])
                        setWarningMessage({ open: true, msg: t("listing:payments.errorThereIsNoTransactionOpt") });
                    }
                    break
                }
                case "multiSelect":
