...
import { Assignment, Group, Notifications, PersonOutline, Schedule } from "@material-ui/icons";
...
import { CCM_USER_BY_USERID_URL } from "app/c1utils/const";
import { deepUpdateState } from "app/c1utils/stateUtils";
import { getValue, hasWhiteSpace, isAccountAdmin, isCustService, isEmpty, isSystemAdmin } from "app/c1utils/utility";
import { encodeString, isArrayNotEmpty } from "app/c1utils/utility";
import withErrorHandler from "app/hoc/withErrorHandler/withErrorHandler";
import useAuth from "app/hooks/useAuth";
import axios from 'axios.js';
import { MOCK_USER_STATUS } from "fake-db/db/clickDOUsers";
import { MatxLoading } from "matx";
import ConfirmationDialog from "matx/components/ConfirmationDialog";

import ManageUserDetails from "./ManageUserDetails";
import ManageUserNotifPref from "./ManageUserNotifPref";
...
const ManageUserFormDetail = () => {
    ...
    let isOfficial = user?.authorities?.some((item) =>
    ...
    const [suspend, setSuspend] = useState({ id: "", open: false, action: null });
    ...
    useEffect(() => {
        if (viewType !== 'new') {
            setLoading(true);
            sendRequest(`${manageUsrUrl}/${isFromProfile ? encodeString(user.id) : encodeString(id)}`, "getUsr", "get", {});
        } else {
            //for new
    ...
    useEffect(() => {
        if (!isLoading && !error && res && !validation) {
            ...
            setNotHoldRoleList(n => {
                if (n?.length == 0)
                    return [...res?.data?.notHoldRoleList || []];
                if (res?.data?.notHoldRoleList === undefined || res?.data?.notHoldRoleList == null)
                    return [];
                return [...n];
            });
            ...
            const toPath = isFromProfile ? "/" : "/opadmin/users";
            ...
            switch (urlId) {
                ...
                case "getUsr":
                    ...
                    if (isFromProfile) {
                        setHoldRoleList([])
                        setNotHoldRoleList([])
                        setNotHoldRoleList(h => ([...h, ...res.data.notHoldRoleList || []]))
                    }
                ...
                case "createUsr":
                    ...
                    break;
                case "getForActive":
                    var portalUser = { "coreUsr": inputData, "holdRoleList": holdRoleList, notifPref: notifPrefData };
                    sendRequest(`${manageUsrUrl}/activate/${isFromProfile ? encodeString(user.id) : encodeString(id)}`, "activate", "PUT", portalUser);
                    break;
                case "activate":
                    setSnackBarOptions({
                        ...snackBarOptions,
                ...
                case "getForDeActive":
                    ...
                    if (isUserLoggedIn) {
                        setConfirm({ ...confirm, id: res?.data?.coreUsr?.usrUid, open: true, action: "deactivate" });
                    } else {
                        setLoading(true);
                        var portalUser = { "coreUsr": inputData, "holdRoleList": holdRoleList, notifPref: notifPrefData };
                        sendRequest(`${manageUsrUrl}/deactivate/${isFromProfile ? encodeString(user.id) : encodeString(id)}`, "deactivate", "put", portalUser);
                    }
                    break;
                case "getForSuspend":
                    if (res?.data?.loggedIn) {
                        setSuspend({ ...suspend, id: res?.data?.coreUsr?.usrUid, open: true, action: "suspend" });
                    } else {
                        setLoading(true);
                        var portalUser = { "coreUsr": inputData, "holdRoleList": holdRoleList, notifPref: notifPrefData };
                ...
                case "getForRoleChange":
                    ...
                    if (isLoggedIn && currHoldRoleList && currHoldRoleList.length !== holdRoleList.length) {
                        setConfirm({ ...confirm, id: res?.data?.coreUsr?.usrUid, open: true, action: "saveUsr" });
                    } else {
                        updateUser();
                    }
                    break;
                ...
                case "usrSuspend":
                    setSnackBarOptions({
                        ...snackBarOptions,
                        success: true,
                        successMsg: t("admin:user.msg.suspended"),
                        redirectPath: toPath
                    });
                    break;
                case "fetchRoles":
    ...
    const handleStatusChange = (action) => {
        if (action === 'resetPassword') {
            ...
            if (action === 'deactivate') {
                sendRequest(`${manageUsrUrl}/${isFromProfile ? encodeString(user.id) : encodeString(id)}`, "getForDeActive", "GET", {});
            } else if (action === 'suspend') {
                sendRequest(`${manageUsrUrl}/${isFromProfile ? encodeString(user.id) : encodeString(id)}`, "getForSuspend", "GET", {});
            } else {
                var portalUser = { "coreUsr": inputData, "holdRoleList": holdRoleList, notifPref: notifPrefData };
                sendRequest(`${manageUsrUrl}/${action}/${isFromProfile ? encodeString(user.id) : encodeString(id)}`, action, "PUT", portalUser);
            }
        }
    }
    ...
    const handleConfirmYesAction = () => {
        ...
        if (confirm?.action === 'deactivate') {
            sendRequest(`/api/v1/clickargo/clictruck/manageusr/deactivate/${isFromProfile ? encodeString(user.id) : encodeString(id)}`, "deactivate", "PUT", portalUser);
        } else {
            updateUser();
        }
    ...
    const handleSuspendYesAction = () => {
        setLoading(true);
        var portalUser = { "coreUsr": inputData, "holdRoleList": holdRoleList, notifPref: notifPrefData };
        if (suspend?.action === 'suspend') {
            sendRequest(`${manageUsrUrl}/update/suspend/${isFromProfile ? encodeString(user.id) : encodeString(id)}`, "usrSuspend", "PUT", portalUser);
        } else {
            updateUser();
        }

        setSuspend({ ...suspend, id: res?.data?.coreUsr?.usrUid, open: false });
    }
    ...
    const updateUser = () => {
        ...
        sendRequest(`${manageUsrUrl}/save/${isFromProfile ? encodeString(user.id) : encodeString(id)}`, "saveUsr", "put", portalUser);
    }

    /**to be triggered when confirmation dialog is OK'd */
    const handleResetPassword = () => {
        var portalUser = { "coreUsr": inputData.coreUsr, "holdRoleList": holdRoleList, "alternateEmail": tempEmail, notifPref: notifPrefData };
        sendRequest(`${manageUsrUrl}/resetPassword/${isFromProfile ? encodeString(user.id) : encodeString(id)}`, "resetPwd", "put", portalUser);
    }
    ...
    const handleSubmit = async (values) => {
        ...
        if (isEmpty(errors)) {
            ...
            switch (viewType) {
                case 'new':
                    sendRequest(`${manageUsrUrl}/create`, "createUsr", "post", portalUser);
                    break;
                case 'edit':
                    //CPEDI-166 Removed the warning message in the Roles tab and replaced with popup confirmation
                    sendRequest(`${manageUsrUrl}/${isFromProfile ? encodeString(user.id) : encodeString(id)}`, "getForRoleChange", "get", {});
                    break;
                default: break;
            }
        } else {
    ...
    const handleValidate = () => {
        ...
        if (viewType === 'new' || viewType === 'edit') {
            ...
            // if (!inputData?.coreUsr?.usrDept)
            //     errors.profile.usrDept = t("common:validationMsgs.required");
            ...
            else if (!inputData?.coreUsr?.usrContact?.contactTel.match("^[+]{1}[(]{0,1}[0-9]{1,4}[)]{0,1}[\\d\\.0-9]*$"))
            ...
            if (errors.holdRoleList)
                setTabIndex(1);
            if (!isEmpty(errors.profile))
                setTabIndex(0);
    ...
    const checkUserId = useCallback(debounce((value) => {
        axios.get(`/api/v1/clickargo/clictruck/manageusr/check-id-availability?usrId=${value}`)
            .then(() => setInputData(inputData => ({ ...inputData, coreUsr: { ...inputData['coreUsr'], isUserIdExists: false } })))
            .catch(() => (setInputData(inputData => ({ ...inputData, coreUsr: { ...inputData['coreUsr'], isUserIdExists: true } }))))
    }, 2000), []);

    const checkContactEmail = useCallback(debounce((id, email) => {
        axios.get(`/api/v1/clickargo/clictruck/manageusr/check-email-availability?usrId=${id}&email=${email}`)
            .then(() => setInputData(inputData => ({ ...inputData, coreUsr: { ...inputData['coreUsr'], isEmailExists: false } })))
            .catch(() => (setInputData(inputData => ({ ...inputData, coreUsr: { ...inputData['coreUsr'], isEmailExists: true } }))))
    }, 500), []);
    ...
    const handleInputChange = async (e) => {
        ...
        if (elName === 'enableContactNo' || elName === 'enableContactEmail' || elName === 'enableTelegramChatId') {
        ...
        if (elName === 'tempEmail') {
            ...
            setNotifPrefData({ ...notifPrefData, [elName]: e.target.checked ? 'Y' : 'N' });
        } else if (elName.includes('accnId')) {
            if (e.target.value !== '') {
                sendRequest(`${manageUsrUrl}/fetchRoles/` + e.target.value, "fetchRoles", "get");
            } else {
                setNotHoldRoleList([]);
            }
            setInputData({ ...inputData, ...deepUpdateState(inputData, elName, e.target.value) });
            ...
            setInputData({ ...inputData, "coreUsr": { ...inputData['coreUsr'], [elName]: e.target.value } });
        } else if (elName === "TCoreAccn.accnId") {
            setInputData({ ...inputData, "coreUsr": { ...inputData['coreUsr'], "TCoreAccn": { "accnId": e.target.value } } });
        } else {
            setInputData({ ...inputData, ...deepUpdateState(inputData, elName, e.target.value) });
            ...
            if (elName === "coreUsr.usrUid") {
                if (e.target.value) {
                    ...
                    setInputData({ ...inputData, ...deepUpdateState(inputData, elName, val) });
                }
                checkUserId(e.target.value);
            } else {
                setInputData({ ...inputData, ...deepUpdateState(inputData, elName, e.target.value) });
    ...
    let elResetPwd = viewType !== 'new' && inputData?.coreUsr?.usrStatus !== RecordStatus.DEACTIVE.code ?
        <C1LabeledIconButton
            tooltip={t("buttons:resetPw")}
            label={t("buttons:reset")}
            action={() => handleStatusChange("resetPassword")}>
            <RotateLeftIcon color="primary" />
        </C1LabeledIconButton> : null

    let formButtons = <C1FormButtons options={{
        back: {
            show: true,
            eventHandler: () => isSysAdmin ? history.goBack() : isFromProfile ? history.push("/") : history.goBack()
        ...
        switch (viewType) {
            case 'view':
                ...
                formButtons = <C1FormButtons options={{
                    ...
                    // Not show for SP_OP_ADMIN
                    // activate: {
                    //     show: (inputData?.coreUsr?.usrStatus === RecordStatus.INACTIVE.code ||
                    //         inputData?.coreUsr?.usrStatus === RecordStatus.SUSPENDED.code) && (!isFromProfile),
                    //     eventHandler: () => handleStatusChange("activate")
                    // },
                    unsuspend: {
                        show: (inputData?.coreUsr?.usrStatus === RecordStatus.INACTIVE.code ||
                            inputData?.coreUsr?.usrStatus === RecordStatus.SUSPENDED.code) && (!isFromProfile),
                        eventHandler: () => handleStatusChange("unsuspend")
                    },
                }} />;
                break;
            ...
            case 'edit':
                ...
                formButtons = <C1FormButtons options={{
                    ...
                    activate: {
                        show: inputData?.coreUsr?.usrStatus === RecordStatus.INACTIVE.code && !isFromProfile,
                        eventHandler: () => handleStatusChange("activate")
                    },
                    deactivate: {
                        show: inputData.coreUsr?.usrStatus === RecordStatus.ACTIVE.code && (!isFromProfile),
                        eventHandler: () => handleStatusChange("deactivate")
                    },
                    suspend: {
                        show: false,
                        eventHandler: () => handleStatusChange("suspend")
                    },
                    save: {
                        show: true,
                        eventHandler: () => handleSubmit()
    ...
    if (isFromProfile) {
        ...
        bc.push({ name: isSysAdmin ? t("user.list.titleAll") : t("user.list.title"), path: isSysAdmin ? "/manageUsers/users/all/list" : "/manageUsers/user/list" });
        bc.push({ name: bcLabel });
    }
    ...
    return (
        loading ? <MatxLoading /> : <React.Fragment>
            {confirm && confirm.open &&
                (<ConfirmationDialog
                    title={confirm.action === "deactivate" ? t("admin:user.deactivate.title") : t("admin:user.roleChange.title")}
                    open={confirm.open}
                    text={confirm.action === "deactivate" ? t("admin:user.deactivate.content",
                        { userId: confirm.id }) : t("admin:user.roleChange.content")}
                    onYesClick={() => handleConfirmYesAction()}
                    onConfirmDialogClose={() => setConfirm({ ...confirm, open: false })} />
                )}

            {suspend && suspend.open &&
                (<ConfirmationDialog
                    title={suspend.action === "suspend" ? t("admin:user.suspend.title") : t("admin:user.roleChange.title")}
                    open={suspend.open}
                    text={suspend.action === "suspend" ? t("admin:user.suspend.content",
                        { userId: suspend.id }) : t("admin:user.roleChange.content")}
                    onYesClick={() => handleSuspendYesAction()}
                    onConfirmDialogClose={() => setSuspend({ ...suspend, open: false })} />
                )}

            <C1FormDetailsPanel
                breadcrumbs={bc}
                titleStatus={![RecordStatus.ACTIVE.code, RecordStatus.DEACTIVE.code, RecordStatus.SUSPENDED.code, RecordStatus.INACTIVE.code].includes(inputData?.coreUsr?.usrStatus) ? RecordStatus.NEW.code : inputData?.coreUsr?.usrStatus}
                title={bcLabel}
                            ...
                            <Paper>
                                <Tabs
                                    ...
                                    {tabList.map((item, ind) => {
                                        ...
                                        // if (!(isAccnAdmin || isSysAdmin || isGDAdmin) && ind === 2)
                                        //     return null;
                                        // else
                                        return <Tab className="capitalize" value={ind} label={item.text} key={ind} icon={item.icon} />
                                    })}
                                </Tabs>
                                ...
                                {tabIndex === 1 && <C1TabInfoContainer><ManageUsersRoles notHoldRoleList={notHoldRoleList}
                                    holdRoleList={holdRoleList}
                                    ...
                                    locale={t}
                                    errors={props.errors} /></C1TabInfoContainer>}
                                {tabIndex === 2 && <C1TabInfoContainer><C1AuditTab filterId={inputData?.coreUsr?.usrUid} /></C1TabInfoContainer>}
                            </Paper>
            ...
            <C1PopUp
                ...
                <Grid container spacing={1} alignItems="center" >
                    ...
                    <Grid container item alignItems="flex-end" spacing={2} direction="row" justify="flex-end">
                        <Grid item xs={6}></Grid>
                        <Grid container item xs={6} spacing={2} direction="row" justify="flex-end">
                            <Grid item xs={3} >
