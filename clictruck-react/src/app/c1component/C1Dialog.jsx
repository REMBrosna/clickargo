...
const C1Dialog = ({ title, isOpen, actionsEl, showActions = true,
    handleSaveEvent, handleCloseEvent,
    children, maxWidth, scroll, isError = false }) => {
        ...
        <React.Fragment>
            <Dialog onClose={handleCloseEvent} aria-labelledby="customized-dialog-title"
                ...
                <DialogTitle id="customized-dialog-title" onClose={() => handleCloseEvent()}>
                    {isError ? <div className="text-center w-360 mx-auto">{title}</div> : title}
                </DialogTitle>
                {!isError ? <DialogContent dividers>
                    {children}
                </DialogContent> : <div className="text-center w-360 mx-auto">{children}</div>}
                <DialogActions>
