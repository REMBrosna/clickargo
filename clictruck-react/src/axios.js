    ...
    (error) => {
        if (error.toJSON().message === 'Network Error') {
            ...
            if ((400 === error.response.status
                && NO_PRINCIPAL_TXT.some(v => error?.response?.data?.err?.msg?.includes(v))) ||
                (400 === error.response.status && error?.response?.data?.err?.msg === sessionCutOff)) {
                return Promise.reject({ err: { msg: sessionTimeout } });
            } else {
                if (error?.response?.data?.err?.msg.includes('attachment')) {
                    return Promise.reject({ err: { msg: error?.response?.data?.err?.msg } });
                } else {
                    //Nina removed displaying of stacktrace for security purposes
                    // return Promise.reject("Something went wrong!");
                    return Promise.reject({ err: { code: error?.response?.data?.err?.code, msg: error?.response?.data?.err?.msg } });
                }
...
export const sessionCutOff = 'com.vcc.camelone.common.exception.ProcessingException: com.vcc.camelone.common.exception.EntityNotFoundException: java.lang.Exception: principal is null'
