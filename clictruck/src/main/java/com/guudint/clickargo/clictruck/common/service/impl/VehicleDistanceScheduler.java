...
@EnableScheduling
@EnableAsync
public class VehicleDistanceScheduler extends AbstractJob implements IMonitorService {

	private static final Logger LOG = Logger.getLogger(VehicleDistanceScheduler.class);
	...
	public void monitor() throws Exception {
		LOG.debug("monitor");
		LOG.info("Scheduler Monitoring Vehicle Distance running in " + InetAddress.getLocalHost().getHostName());
		...
		try {
			...
			LOG.info("Monitoring Vehicle Distance Started: " + Calendar.getInstance().getTime().toString());
			try {
				executeMonitoringVehicleDistance();
			...
			LOG.info("Monitoring Vehicle Distance Ended: " + Calendar.getInstance().getTime().toString());
	...
	public void executeMonitoringVehicleDistance() throws Exception {
		LOG.info("executeMonitoringVehicleExpiry");

		// Get the current date formatted
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Date currentDate = new Date();
		List<String> listState = Arrays.asList(NotificationTemplateName.VEHICLE_MAINTENANCE.getDesc(), ClickargoNotifTemplates.VEH_DISTANCE.getId());
			...
			for (TCkCtAlert tCkCtAlert : listAlerts) {
				...
				if ("WHATSAPP".equalsIgnoreCase(notificationType) && tCkCtAlert.gettCoreAccn() != null) {
					...
					if (whatsappNumber != null) {
						...
						text.add(ckCtAlert.getAltConditionValue());
						...
						jobDeliveryService.sendYCloudWhatAppMsg(
								...
								text,
								null,
								NotificationTemplateName.VEHICLE_TOTAL_DISTANCE_TRAVELED.getDesc()
						);
					}
				}
...
//	public void executeMonitoringVehicleDistance() throws Exception {
//		LOG.info("executeMonitoringVehicleDistance");
//		List<TCkCtVehExt> listVehExt = ckCtVehExtDao.findByMonitoring(VehExtParamEnum.VEHICLE_MAINTENANCE.name(), 'O', null);
...
//					LOG.info("Sending email notification to monitoring vehicle distance: "
...
//					String message = "monitoring vehicle distance " + tCkCtVehExt.getId().getVextId();
//					LOG.info("Sending whatsapp notification to monitoring vehicle distance: "
...
//					text.add(tCkCtVehExt.getVextMonitorValue());
//					jobDeliveryService.sendYCloudWhatAppMsg(null, tripMobileService.removeSpecialCharacters(tCkCtVehExt.getVextNotifyWhatsapp()), text,
//							null, YCloudTemplateName.VEHICLE_TOTAL_DISTANCE_TRAVELED.getDesc());
//				}
//			}
//		}
//	}
