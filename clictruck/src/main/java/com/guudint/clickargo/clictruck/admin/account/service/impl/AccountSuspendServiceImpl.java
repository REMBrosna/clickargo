...
@Service
public class AccountSuspendServiceImpl extends AccnService {
	
	private static Logger LOG = Logger.getLogger(AccountSuspendServiceImpl.class);
	...
	public CoreAccn updateStatus(AccountSuspend accountSuspend)
		...
		if (RecordStatus.SUSPENDED.getCode() == accountSuspend.getAccnStatus()) {
			coreAccn.setAccnStatus(accountSuspend.getAccnStatus());
			coreAccn.setAccnDtSusp(new Date());
			
			suspLog.setSlDetails("Manual Suspension");
		...
		if (Character.isAlphabetic(coreAccn.getAccnStatus())) {
			...
			if (((AccountSuspend) coreAccn).getHistory() != null
				...
				parameters.put("validStatus",
						Arrays.asList(RecordStatus.SUSPENDED.getCode(), RecordStatus.DEACTIVATE.getCode()));
			}
		}
