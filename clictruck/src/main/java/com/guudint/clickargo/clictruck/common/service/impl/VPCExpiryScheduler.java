...
@EnableScheduling
@EnableAsync
public class VPCExpiryScheduler extends AbstractJob implements IMonitorService {

	private static final Logger LOG = Logger.getLogger(VPCExpiryScheduler.class);
	...
	public void monitor() throws Exception {
		LOG.debug("monitor");
		LOG.info("Scheduler Monitoring VPC Expiry running in " + InetAddress.getLocalHost().getHostName());
		...
		try {
			...
			LOG.info("Monitoring VPC Expiry Started: " + Calendar.getInstance().getTime().toString());
			try {
				executeMonitoringVPCExpiry();
			...
			LOG.info("Monitoring VPC Expiry Ended: " + Calendar.getInstance().getTime().toString());
	...
	public void executeMonitoringVPCExpiry() throws Exception {
		LOG.info("executeMonitoringVPCExpiry");

		// Get the current date formatted
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Date currentDate = new Date();
		List<String> listState = Arrays.asList(NotificationTemplateName.VEHICLE_PARKING_CERT_EXPIRED.getDesc(), ClickargoNotifTemplates.VPC_EXPIRY.getId());
				...
				if ("WHATSAPP".equalsIgnoreCase(notificationType) && tCkCtAlert.gettCoreAccn() != null) {
					...
					if (whatsappNumber != null) {
						...
						jobDeliveryService.sendYCloudWhatAppMsg(
								...
								text,
								null,
								NotificationTemplateName.VEHICLE_PARKING_CERT_EXPIRED.getDesc()
						);
					}
				}
...
//
//		List<TCkCtVehExt> listVehExt = ckCtVehExtDao.findByMonitoring(VehExtParamEnum.EXP_VPC_EXPIRY.name(), 'D', formattedDate);
...
//					LOG.info("Sending email notification to monitoring vpc expiry: "
...
//					String message = "monitoring vpc expiry " + tCkCtVehExt.getId().getVextId();
//					LOG.info("Sending whatsapp notification to monitoring vpc expiry: "
...
//					ArrayList<String> text = new ArrayList<>();
//					text.add(ckCtVehExt.getTCkCtVeh().getTCoreAccn().getAccnName());
//					text.add(ckCtVehExt.getTCkCtVeh().getVhPlateNo());
...
//							null, YCloudTemplateName.VEHICLE_PARKING_CERT_EXPIRED.getDesc());
//				}
//			}
//		}
//	}
