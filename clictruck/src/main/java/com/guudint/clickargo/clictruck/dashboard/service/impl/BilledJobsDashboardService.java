...
public class BilledJobsDashboardService implements IDashboardStatisticsService {
	...
	public DashboardJson getStatistics(DashboardJson dbJson, Principal principal) throws Exception {
		...
		params.put("activeStates", Arrays.asList(JobStates.BILLED.name(), JobStates.VER_BILL.name()));

		dbJson.getTransStatistic().put(DashboardStatus.PENDING_VERIFICATION.name(),
