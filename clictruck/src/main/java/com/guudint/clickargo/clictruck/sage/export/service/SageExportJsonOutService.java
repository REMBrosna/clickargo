...
@Deprecated
@Service
public class SageExportJsonOutService extends SageExportJsonService {

	// Static Attributes
	////////////////////
	private static Logger LOG = Logger.getLogger(SageExportJsonOutService.class);
	...
	protected List<SageExportBookingDto> createSageExportDto(String dateFolderName, Date beginTime, Date endTime)
		...
		List<TCkPaymentTxn> payTxtList = ckPaymentTxnDao.getByPaymentTypePaidAndStatus(
				PaymentTypes.BANK_TRANSFER.getId(), beginTime, endTime, Constant.ACTIVE_STATUS);
		...
		if (payTxtList != null && payTxtList.size() > 0) {
			...
			for (int id = 0; id < payTxtList.size(); id++) {
				...
				String fileName = dateFolderName + "PO-" + String.format("%03d", id) + ".json";
				//
				...
				CkCtSageExportOutJson sageExportBookingJson = new CkCtSageExportOutJson(SAGE_EXPORT_JSON_SERVICE,
						SAGE_EXPORT_JSON_TYPE_PAY_OUT, payTxn.getPtxId(), paiddDate, total, provider);
	...
	private Provider getProvider(TCkPaymentTxn payTxt) throws Exception {
		...
		for (String jobTruckId : jobTruckIds) {
			...
			// ConsumerProviderBooking consumer = generateConsumerProviderBooking(coAccnId,
			...
			ConsumerProviderBooking provider = generateConsumerProviderBooking(toAccnId, invList, debitNoteList, true);

			toSageId = provider.id;

			itemList.add(new Item(provider.debitNote, provider.invoice));
		}
