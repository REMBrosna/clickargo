 ...
 */
public class CkJobTruckASGListService extends AbstractCkListingService<TCkJobTruck, String, CkJobTruckMobileDto> {
	...
	public CkJobTruckASGListService() {
	...
	protected CkJobTruckMobileDto dtoFromEntity(TCkJobTruck entity) throws ParameterException, ProcessingException {
		...
		try {
			...
			if (tckCtTrips != null) {
				...
				jobMtrips.stream().sorted(Comparator.comparing(CkJobMTripDto::getSeqNo));
	...
	protected HashMap<String, Object> getParameters(CkJobTruckMobileDto dto)
		...
		try {
			...
			if (dto.getHistory() != null && dto.getHistory().equalsIgnoreCase(DEFAULT)) {
				parameters.put("jobTruckStates", JobStates.ASG.name());
			}
	...
	@Transactional
	public CkJobTruck startAssignJob(String jobId) throws Exception {
		LOG.debug("startAssignJob");
		...
		try {
			// 1: Change all ONGOING status job to PAUSE status
			List<TCkJobTruck> listJobTruck = ckJobTruckDao.findByDrvMobileId(principal.getUserId(),
					Arrays.asList(JobStates.ONGOING.name()));
			...
			if (opTCkJobTruckNew.isPresent()) {
				...
				List<CkCtTrip> tckCtTrips = ckCtTripService.findTripsByTruckJobAndStatus(jobId,
						Arrays.asList(TripStatus.M_ACTIVE.getStatusCode(), TripStatus.M_PICKED_UP.getStatusCode(),
								TripStatus.M_DELIVERED.getStatusCode(), TripStatus.DLV.getStatusCode()));

				// 2.2 Update all trip start time.
				for (CkCtTrip ckCtTrip : tckCtTrips) {
					Optional<TCkCtTripLocation> opTripLoc = Optional
							.ofNullable(ckCtTripLocationDao.find(ckCtTrip.getTCkCtTripLocationByTrFrom().getTlocId()));
					if (opTripLoc.isPresent()) {
						opTripLoc.get().setTlocDtStart(new Date());
						ckCtTrip.getTCkCtTripLocationByTrFrom().setTlocDtStart(new Date());
						ckCtTripLocationDao.saveOrUpdate(opTripLoc.get());
					}
				}

				// 2.3 Update Job start time
				Optional<CkRecordDate> ckRecordDate = Optional.ofNullable(
						ckRecordService.findById(opTCkJobTruckNew.get().getTCkJob().getTCkRecordDate().getRcdId()));
				
				if (ckRecordDate.isPresent() && ckRecordDate.get().getRcdDtStart() == null) {
					ckRecordDate.get().setRcdDtStart(new Date());
					ckRecordService.update(ckRecordDate.get(), principal);
				}
			}
			return ckJobTruck;
		} catch (Exception e) {
			LOG.error("startAssignJob", e);
