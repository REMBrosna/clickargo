...
const BasketButton = ({ isWhite, style, navigation }) => {
    console.log("theme.SIZES.BASE: ", theme.SIZES.BASE);
    return <TouchableOpacity style={[styles.button, style]} onPress={() => navigation.navigate('Cart')}>
        <Icon
            family="GalioExtra"
            size={16}
            name="basket-simple"
            color={theme.COLORS[isWhite ? 'WHITE' : 'ICON']}
        />
        <Block middle style={[styles.notify, { backgroundColor: "green" }]} >
            <Text>2</Text>
        </Block>
    </TouchableOpacity>
};
        ...
        return (
            <Block row style={styles.tabs}>
                <Button shadowless style={[styles.tab, styles.divider]} onPress={() => navigation.navigate('Categories')}>
                    <Block row middle>
                        <Icon name="person-add" family="ionicon" style={{ paddingRight: 5 }} />
    ...
    renderTabs = () => {
        ...
        return (
            <Tabs
                ...
                onChange={id => {
                    console.log("id: ", id);
                    return navigation.setParams({ tabId: id })
                }} />
        )
    }
